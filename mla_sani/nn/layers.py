...
class Dense(Layer):
    ...
    def build(self, input_shape, optimizer=None):
        # initialize weights
        self.kernel = np.random.randn(input_shape, self.output_shape)
...
class Conv2D(Layer):
    ...
    The process is almost as simple as Dense layer, except the `im2col` before forward and the
    `col2im` after backprop.  However, the shape thing is fairly complex.
